...
class Users_model extends Crud_model {
    ...
    function authenticate($email, $password) {
        $this->db->select("id,user_type,client_id,has_timer,task_id,project_id");
        ...
        if ($result->num_rows() == 1) {
            ...
            if($user_info->has_timer === "1"){
                //set session if timer is found
                $timer_session = array(
    ...
    function save_timer($data) {
        parent::use_table("users");

        $where = array("id" => get_array_value($data, "id"));
        $exists = parent::get_one_where($where);
        if ($exists->id) {
            //user found update timer
            return parent::update_where($data, $where);
        }
    }
